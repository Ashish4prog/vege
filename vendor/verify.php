...
if(isset($_REQUEST['username']) && !empty($_REQUEST['username']) AND isset($_REQUEST['hash']) && !empty($_REQUEST['hash']))
{
	...
	$search = mysql_query("SELECT username, hash, active FROM vendorlogin WHERE username='".$username."' AND hash='".$hash."' AND active='0'") or die(mysql_error()); 
    $match  = mysql_num_rows($search);
                 
    if($match > 0){
        // We have a match, activate the account
        mysql_query("UPDATE vendorlogin SET active='1' WHERE username='".$username."' AND hash='".$hash."' AND active='0'") or die(mysql_error());
